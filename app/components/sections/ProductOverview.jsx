...
const ProductOverview = () => {
  ...
  const categories = ['All Products', 'clothes', 'Bracelet', 'Watches'];
  ...
  const filteredProducts = activeTab === 'All Products' 
    ? products 
    : products.filter(p => {
        const categoryMap = { Clothes: 'Women', Bracelet: 'Bracelet', Watches: 'Watches' };
        const key = categoryMap[activeTab] || activeTab;
  ...
  const handleLoadMore = () => {
    if (activeTab === 'All Products') {
      router.push('/newarrivals'); // All Products -> New Arrivals page
    } else {
      const categoryMap = { Clothes: 'Women', Bracelet: 'Bracelet', Watches: 'Watches' };
      const key = categoryMap[activeTab] || activeTab;
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-14">
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xl md:text-2xl font-bold text-black pt-1">${product.price}</p>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        <div className="flex justify-center mt-10">
          <button
            ...
            className="px-6 py-2 bg-gray-300 text-white rounded-md hover:bg-gray-800 transition-all"
          >
            Load More
          </button>
        </div>
